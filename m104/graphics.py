...
class GraphWin(tk.Canvas):
    ...
    def __init__(self, title="Graphics Window",
                 width=200, height=200, pos_x=-1024, pos_y=0, autoflush=True):
...
#        master.geometry("{0}x{1}+1920+0".format(
#            master.winfo_screenwidth()-pad, master.winfo_screenheight()-pad))
       # master.geometry("1015x736+0+0")        
        master.geometry('{0}x{1}+{2}+{3}'.format(width, height, pos_x, pos_y))
...
##//     
    ...
    def roll_down(self):
        geom = self.master.winfo_geometry()
        size_x, rest = geom.split('x', 1)
        size_y, rest = rest.split('+', 1)
        pos_x, pos_y = rest.split('+', 1)
        size_x = int(size_x)
        size_y = int(size_y)
        pos_x = int(pos_x)
        pos_y = int(pos_y)
        while pos_y < 1200:
            geom = '{}x{}+{}+{}'.format(size_x, size_y, pos_x, pos_y)
            pos_y += 5
